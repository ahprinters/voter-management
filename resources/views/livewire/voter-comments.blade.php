                    ...
                    <div class="flex justify-between items-start">
                        <div class="flex-1">
                            <div class="flex items-center gap-2 mb-2">
                                <h4 class="font-bold text-gray-900 text-lg">{{ $comment->title ?? 'শিরোনামহীন মন্তব্য' }}</h4>
                            ...
                            <p class="text-xs text-gray-500 mb-3 flex items-center">
                                ...
                                {{ $comment->created_at->format('d M, Y - h:i A') }} ({{ $comment->created_at->diffForHumans() }})
                            </p>
                        ...
                        {{-- এডিট ও ডিলিট বাটনসমূহ --}}
                        <div class="flex items-center gap-1 ml-4">
                            <button wire:click="$call('editComment', {{ $comment->id }})" type="button" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition" title="এডিট">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                </svg>
                            </button>
                            <button wire:click="deleteComment({{ $comment->id }})" wire:confirm="আপনি কি নিশ্চিত?" type="button" class="p-2 text-red-600 hover:bg-red-50 rounded-lg transition" title="ডিলিট">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @if ($comment->file_path)
                        <div class="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between">
                            <div class="flex items-center text-sm text-gray-600">
                                ...
                                সংযুক্ত ফাইল: <span class="ml-1 font-medium">{{ strtoupper($comment->file_type) }}</span>
                            </div>
                            <a href="{{ asset('storage/' . $comment->file_path) }}" target="_blank" class="inline-flex items-center px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-bold rounded-lg">ডাউনলোড/দেখুন</a>
                        </div>
                    @endif
                @endif
            </div>
